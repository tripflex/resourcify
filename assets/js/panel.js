...
function resourcify_randomUUID() {
		var s = [], itoh = '0123456789ABCDEF';
		for (var i = 0; i <6; i++) s[i] = Math.floor(Math.random()*0x10);
		return s.join('');
}
var resourcify_field_callbacks = [];
jQuery('document').ready(function($){
	// add row
	$('body').on('click', '.resourcify-add-group-row', function(){
		var clicked = $( this ),
			rowid = resourcify_randomUUID(),
			...
			for(var callback in resourcify_field_callbacks){
				if( typeof window[resourcify_field_callbacks[callback]] === 'function'){
					window[resourcify_field_callbacks[callback]]();
				}
			}

	});
	$('body').on('click', '.resourcify-removeRow', function(){
		$(this).next().remove();
		$(this).remove();
		////console.log(this);
	});
