...
class Resourcify {
	 ...
	 * @var      string
	 */
	protected $plugin_slug = 'resourcify';
	/**
	 * @var      object
	 */
	...
	function add_metaboxes( $slug, $post = false ){
		...
		if(!empty($post)){
			if(!in_array($post->post_type, array('post'))){return;}
				...
				$screen = get_current_screen();
				if(!in_array($screen->base, array('post'))){return;}
			}
	...
	function render_metaboxes($post, $args){
		...
		echo '<input type="hidden" name="resourcify_metabox" id="resourcify_metabox" value="'.wp_create_nonce(plugin_basename(__FILE__)).'" />';
		echo '<input type="hidden" name="resourcify_metabox_prefix[]" value="'.$args['args']['slug'].'" />';
		...
		if( count( $groups ) > 1){
			if(!empty($post)){
				$current_tab = get_post_meta($post->ID, 'resourcify__cur_tab__', true);
			}else{
				$current_tab = get_option('resourcify__cur_tab__');
			}

			if(empty($current_tab)){
				$current_tab = 0;
			}
			echo "<input type=\"hidden\" name=\"resourcify[__cur_tab__]\" id=\"resourcify__cur_tab__\" value=\"".(!empty($current_tab) ? $current_tab : 0)."\">";
			echo "<div class=\"resourcify-metabox-config-nav\">\r\n";
			echo "	<ul>\r\n";
			foreach ($groups as $key=>$group) {
					echo "		<li class=\"" . ( !empty($current_tab) ? ($$current_tab == $key ? "current" : "") : ($key === 0 ? "current" : "" )) . "\">\r\n";
					echo "			<a data-tabkey=\"".$key."\" data-tabset=\"resourcify__cur_tab__\" title=\"".$group['label']."\" href=\"#".$group['id']."\"><strong>".$group['label']."</strong></a>\r\n";
					echo "		</li>\r\n";
			}
			echo "	</ul>\r\n";
			echo "</div>\r\n";
		...
		// build instance
		foreach ($groups as $key=>$group) {
			echo "<div class=\"resourcify-metabox-config-content " . $panelsetsize . " group\" id=\"".$group['id']."\" ". ( $current_tab == $key ? '' : 'style="display:none;"' ) .">\r\n";
			...
			for( $i=0; $i<$depth;$i++ ){
				...
				if($i > 0){
					echo '  <div class="button button-primary right resourcify-removeRow" style="margin-top:5px;">'.__('Remove', 'resourcify').'</div>';
				}
			...
			if(!empty($group['multiple'])){
				echo "<div class=\"toolrow\"><button class=\"button resourcify-add-group-row\" type=\"button\" data-rowtemplate=\"group-".$args['id']."-tmpl\">".__('Add Another', 'resourcify')."</button></div>\r\n";
			}
			echo "</div>\r\n";
	...
	public function get_post_meta($id, $key = null, $single = false){
		...
		if(!empty($key)){
			...
			if(file_exists(self::get_path( __FILE__ ) .'configs/fieldgroups-resourcify.php')){
				include self::get_path( __FILE__ ) .'configs/fieldgroups-resourcify.php';		
			}else{
				return;
			}
	...
	function save_post_metaboxes($pid, $post){
		...
		if(!isset($_POST['resourcify_metabox']) || !isset($_POST['resourcify_metabox_prefix'])){return;}


		if(!wp_verify_nonce($_POST['resourcify_metabox'], plugin_basename(__FILE__))){
			return $post->ID;
		}
		...
		foreach( $_POST['resourcify_metabox_prefix'] as $prefix ){
			if(!isset($_POST[$prefix])){continue;}

			
			delete_post_meta($post->ID, $prefix);
			add_post_meta($post->ID, $prefix, $_POST[$prefix]);
			//foreach($_POST['resourcify'] as $field=>$data){
				
				//if(is_array($data)){
				//	foreach($data as $item){
				//		add_post_meta($post->ID, 'resourcify_'.$field, $item);
				//	}
				//}else{
	...
	public function render_element($atts, $content, $slug, $head = false) {
		...
		if(!empty($head)){
			$instanceID = $this->element_instance_id('resourcify'.$slug, 'header');
		}else{
			$instanceID = $this->element_instance_id('resourcify'.$slug, 'footer');
		}
